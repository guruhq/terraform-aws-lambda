...
locals {
  ...
  deploying_lambda     = local.create && var.create_function && var.manage_deployments && !var.create_layer ? aws_lambda_function.this[0] : null
  non_deploying_lambda = local.create && var.create_function && !var.manage_deployments && !var.create_layer ? aws_lambda_function.ignore_deployments[0] : null
  lambda_function      = local.deploying_lambda != null ? local.deploying_lambda : local.non_deploying_lambda
}

resource "aws_lambda_function" "this" {
  count = local.create && var.create_function && var.manage_deployments && !var.create_layer ? 1 : 0
...
resource "aws_lambda_function" "ignore_deployments" {
  count = local.create && var.create_function && !var.manage_deployments && !var.create_layer ? 1 : 0

  lifecycle {
    ignore_changes = [image_uri, package_type, filename, source_code_hash, s3_bucket, s3_key, s3_object_version]
  }
...
resource "aws_lambda_provisioned_concurrency_config" "current_version" {
  ...
  qualifier     = local.lambda_function.version
...
resource "aws_lambda_function_event_invoke_config" "this" {
  ...
  function_name = local.lambda_function.function_name
  qualifier     = each.key == "current_version" ? local.lambda_function.version : null
...
resource "aws_lambda_function_url" "this" {
  ...
  qualifier          = var.create_unqualified_alias_lambda_function_url ? null : local.lambda_function.version
...
resource "null_resource" "sam_metadata_aws_lambda_function" {
  ...
  triggers = {
    ...
    # resource
    resource_name = "local.lambda_function"
